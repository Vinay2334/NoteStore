...
class UserProfileSerializer(serializers.ModelSerializer):
  ...
  confirm_password = serializers.CharField(write_only=True, style={'input_type': 'password'}, min_length=5)

  class Meta:
    model = models.UserProfile
    fields = [
        'id', 'email', 'name', 'password', 'confirm_password', 'college_name',
        'profile_pic', 'otp', 'total_uploads'
    ]
    extra_kwargs = {
        'password': {
            'write_only': True,
  ...
  def update(self, instance, validated_data):
    ...
    if email and otp:
      ...
      if not generated_otp:
        raise serializers.ValidationError({'error': 'OTP not generated'})
      if generated_otp != otp:
        raise serializers.ValidationError({'error': 'OTP does not match'})
    elif email:
